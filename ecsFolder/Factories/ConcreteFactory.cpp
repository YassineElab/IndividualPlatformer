...
Entity ConcreteFactory::createEnemy(World &world, float x, float y, float width, float height, sf::Vector2f A,
                                    sf::Vector2f B) {
    Entity enemy = world.createEntity();

    world.positions[enemy]   = Position{x, y};
    world.transforms[enemy]  = Transform{ .position = {x, y} };
    world.sizes[enemy]       = Size{width, height};
    world.velocities[enemy]  = Velocity{0.f, 0.f};
    world.renderables[enemy] = Renderable({width, height}, "../ecsFolder/Resources/Pics/ufo.png");
    world.colliders[enemy]   = Collider{ .bounds = {x, y, width, height}, .isStatic = false };

    world.enemies[enemy] = Enemy{
            .state = &PatrolState::instance(),
            .anchorA = A,
            .anchorB = B,
            .respawnPos = {x, y}
    };

    world.enemyTags.insert({enemy, EnemyTag{}});
